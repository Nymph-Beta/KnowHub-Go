...
STAGE12_SCRIPT="${SCRIPT_DIR}/stage12_acceptance.sh"
...
usage() {
  cat <<'EOF'
Usage:
  run_acceptance.sh -s <6|7|6-9|10|11|12|all> [options]

Options:
  -s <stage>       指定阶段：6 | 7 | 6-9 | 10 | 11 | 12 | all（必填）
...
Examples:
  ...
  ./scripts/acceptance/run_acceptance.sh -s 12 --stage7-file ./scripts/test_data/Hashimoto.pdf -u test -p 123456
...
[[ -n "${STAGE}" ]] || fail "必须指定 -s <6|7|6-9|10|11|12|all>"
...
[[ -x "${STAGE12_SCRIPT}" ]] || fail "缺少脚本: ${STAGE12_SCRIPT}"
...
case "${STAGE}" in
  ...
  all)
    ...
    ;;
  *)
    fail "无效 stage: ${STAGE}（仅支持 6 | 7 | 6-9 | 10 | 11 | 12 | all）"
    ;;
esac
